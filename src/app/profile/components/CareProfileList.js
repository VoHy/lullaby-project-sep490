import { FaPhone, FaMapMarkerAlt, FaStickyNote, FaUsers } from "react-icons/fa";

export default function CareProfileList({ careProfiles, relativesList, zones }) {
  if (!careProfiles || careProfiles.length === 0) {
    return (
      <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
        <div className="text-6xl mb-4">📋</div>
        <h3 className="text-xl font-semibold text-gray-800 mb-2">Chưa có hồ sơ chăm sóc</h3>
        <p className="text-gray-600 mb-4">Bạn chưa có hồ sơ chăm sóc nào.</p>
      </div>
    );
  }
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {careProfiles.map(care => (
        <div key={care.CareProfileID} className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow">
          ...
          <div className="mt-6 pt-4 border-t border-gray-100">
            <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
              ...
              Con cái ({relativesList.filter(r => r.CareProfileID === care.CareProfileID).length})
            </h4>
            {relativesList.filter(r => r.CareProfileID === care.CareProfileID).length === 0 ? (
              ...
              <div className="space-y-2">
                {relativesList.filter(r => r.CareProfileID === care.CareProfileID).map(r => (
                  <div key={r.RelativeID} className="bg-gray-50 rounded-lg p-3">
