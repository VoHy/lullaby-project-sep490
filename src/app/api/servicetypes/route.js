...
export async function GET() {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/servicetypes/getall`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: errorData.message || 'Không thể lấy danh sách service types' },
...
export async function POST(request) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/servicetypes`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = JSON.parse(errorText);
        return NextResponse.json(
          { error: errorData.message || 'Tạo service type thất bại' },
          { status: response.status }
        );
      } catch (parseError) {
        console.error('Failed to parse error response as JSON:', parseError);
