...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/Booking/GetAllByStatus?status=${status}`, {
      method: 'GET',
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json({ error: errorData.message || 'Không thể lấy danh sách bookings theo status' }, { status: response.status });
      } catch (parseError) {
