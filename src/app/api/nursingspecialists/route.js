  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json({ error: errorData.message || 'Không thể lấy danh sách nursing specialists' }, { status: response.status });
      } catch (parseError) {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json(
          { error: errorData.message || `Tạo nursing specialist thất bại` },
