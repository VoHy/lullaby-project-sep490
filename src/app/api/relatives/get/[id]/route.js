...
export async function GET(request, { params }) {
  try {
    ...
    const response = await fetch(`${backendUrl}/api/relatives/get/${id}`, {
      method: 'GET',
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return NextResponse.json({ error: errorData.message || 'Không thể lấy relative' }, { status: response.status });
      } catch (parseError) {
