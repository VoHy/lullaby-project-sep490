    ...
    if (detailData) {
      ...
      try {
        ...
        if (assignments.length > 0) {
          ...
          // After all assignments, send a single booking-level notification to the customer
          try {
            const customerAccountId = detailData?.careProfile?.accountID || detailData?.careProfile?.accountId || detailData?.careProfile?.AccountID;
            if (customerAccountId) {
              await notificationService.createNotification({
                accountID: customerAccountId,
                message: `Lịch hẹn #${detailData.bookingID} đã được cập nhật. Vui lòng xem lịch sử lịch hẹn để biết thêm chi tiết.`
              });
            }
          } catch (notifyErr) {
            console.warn('Không thể gửi thông báo booking-level cho customer sau khi gán nhân sự:', notifyErr);
          }
        }
